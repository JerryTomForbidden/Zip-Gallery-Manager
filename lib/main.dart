...
import 'package:exzip_manager/screens/home.dart';
    ...
    return MultiProvider(
      providers: [
        ...
        ChangeNotifierProxyProvider<PrefProvider, TagProvider>(
          update: (context, pref, previousTag) {
            print('updated tag provider : ${previousTag!.initialized}');
            return previousTag.initialized ? previousTag : TagProvider();
          },
          create: (context) {
            print('on Create tag provider');
            return TagProvider();
          },
        ),
      ],
      child: Consumer<PrefProvider>(
        builder: (context, pref, child) => MaterialApp(
          title: 'Flutter Demo',
          theme: ThemeData(
          ...
          home: pref.loaded
              ? HomeScreen()
              : FutureBuilder(
                  future: pref.loadPrefs(),
                  builder: (context, snapshot) =>
                      snapshot.connectionState == ConnectionState.waiting
                          ? Scaffold(body: Center(child: Text('waiting...')))
                          : HomeScreen()),
          routes: {
            ...
            TagDetailScreen.routeName: (ctx) => TagDetailScreen(),
            GalleryReaderScreen.routeName: (ctx) => GalleryReaderScreen(),
          },
...
class _FilePickerDemoState extends State<FilePickerDemo> {
  ...
  List<FileSystemEntity>? _directoryFiles;
  ...
  void _selectFolder() {
    FilePicker.platform.getDirectoryPath().then((value) {
      _directoryFiles = [];
      _scanning = true;
      Directory(value!).list(recursive: false, followLinks: false).listen(
          (file) {
        _directoryFiles!.add(file);
      }, onDone: () {
        _scanning = false;
        print(_directoryFiles!.length);
        setState(() {
                          ...
                          ? Column(
                              children: [
                                ListTile(
                                  ...
                                  subtitle: Text(
                                      '${_directoryPath!} ${_directoryFiles!.length}'),
                                ),
                              ],
                            )
                          : _paths != null
